const service = require('./finding.service');
...
exports.reopenFinding = async (req, res, next) => { try { response.success(res, await service.reopenFinding(req.params.id, req.user.scopeOrgId)); } catch (e) { next(e); } };
exports.getByAudit = async (req, res, next) => { try { response.success(res, await service.getByAudit(req.params.auditId, req.user.scopeOrgId)); } catch (e) { next(e); } };
