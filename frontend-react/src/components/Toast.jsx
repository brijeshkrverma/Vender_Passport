...
export function ToastProvider({ children }) {
  ...
  return (
    <ToastContext.Provider value={{ toast }}>
      {children}
      <div style={{position:'fixed',bottom:22,right:22,zIndex:200,display:'flex',flexDirection:'column',gap:8}} role="alert" aria-live="assertive">
        {toasts.map(t => {
          const colors = {success:'#1F7A4D',warning:'#B4650B',error:'#B0362A',info:'#2E5F82'};
          const iconBg = colors[t.type] || '#1F7A4D';
          return (
            <div key={t.id} style={{background:'#0F1B2D',color:'#fff',padding:'13px 16px',borderRadius:9,boxShadow:'0 12px 40px rgba(15,27,45,.16)',fontSize:13,display:'flex',alignItems:'center',gap:10,minWidth:280,animation:'toastIn .18s ease'}}>
